# ============================================
# KỊCH BẢN 3: PPO + ESS + DR (PPO-ESS+DR)
...
# ====================================================
# 1. MÔI TRƯỜNG DRL CHO KỊCH BẢN PPO + ESS + DR
# ====================================================
class IEEE33EnvPPOESSDR(gym.Env):
    """
    Môi trường DRL cho Microgrid IEEE 33-bus:
    - Nguồn: PV, gió (wind) theo profile 24h
    - ESS với ràng buộc SoC, công suất, hiệu suất
    - DR tác động lên một phần phụ tải (flex load)
    - Ràng buộc điện áp: 0.95–1.05 pu
    - Reward đa mục tiêu: chi phí lưới, PV curtail, điện áp, ESS, DR, ramping
    """
    ...
    def __init__(self, seed: int = 1):
        ...
        # ----- Cấu hình DR -----
        # Một phần phụ tải là linh hoạt (ví dụ 30%)
        self.dr_flex_ratio = 0.3
        self.dr_max = 0.1  # ±10%

        # ----- Action space -----
        # [a_ESS, a_PV_curtail, a_DR]
        #  a_ESS      ∈ [-1, 1]   => P_ESS ∈ [-P_max, +P_max]
        #  a_PV_cur   ∈ [ 0, 1]   => PV_curtail_ratio
        #  a_DR       ∈ [-1, 1]   => DR_level ∈ [-dr_max, +dr_max]
        ...
        self.w_dr_usage    = 0.2   # mức sử dụng DR
    ...
    def step(self, action):
        a_ess, a_pv_curtail, a_dr = np.array(action, dtype=np.float32)

        # Clip action
        a_ess = float(np.clip(a_ess, -1.0, 1.0))
        a_pv_curtail = float(np.clip(a_pv_curtail, 0.0, 1.0))
        a_dr = float(np.clip(a_dr, -1.0, 1.0))
        ...
        # Phụ tải sau DR: chỉ 1 phần tải là linh hoạt
        alpha_t = float(self.alpha_profile[self.t])
        base_p_t = self.base_p * alpha_t
        base_q_t = self.base_q * alpha_t

        dr_level = a_dr * self.dr_max  # ∈ [-dr_max, dr_max]
        # DR chỉ tác động lên phần flex
        flex_factor = 1.0 + dr_level * self.dr_flex_ratio
        flex_factor = float(np.clip(flex_factor, 0.7, 1.3))

        self.net.load.p_mw = base_p_t * flex_factor
        self.net.load.q_mvar = base_q_t * flex_factor
...
# ====================================================
# 2. HUẤN LUYỆN PPO CHO KỊCH BẢN PPO-ESS+DR
# ====================================================
if __name__ == "__main__":
    seed = GLOBAL_SEED

    # Tạo env vector hóa – KHÔNG gọi reset(seed=...) trên DummyVecEnv
    env = DummyVecEnv([lambda: IEEE33EnvPPOESSDR(seed=seed)])
    ...
    # ====================================================
    # Không truyền seed vào DummyVecEnv.reset để tránh lỗi
    obs = env.reset()
    rows = []
    ...
    df.to_csv("scenario3_ppo_ess_dr.csv", index=False, encoding="utf-8-sig")
    print("Saved results to scenario3_ppo_ess_dr.csv")
